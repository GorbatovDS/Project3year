...
def wait(name):
    """
    while evaluation of the route is not done (evaluating thread is not dead) render loading screen

    :param name: name of evaluating thread
    :return: None
    """
...
def done():
    """
    show each route on the map (one map for one day)
...
def register():
    """
    save users choices (city, transport type, places) and then start evaluation

    :return: None
    """
    global city, places, tr_choice
    tt = {'car': 'Машина', 'cycle': 'Велосипед/мотоцикл', 'foot': 'Пешком'}
    your_position = (55.803139, 37.409651)
    ...
    if request.method == 'POST':
        ...
        if city and tr_choice and places:
            t = threading.Thread(target=find_route, args=(places, your_position, tr_choice), daemon=False)
            t.start()
